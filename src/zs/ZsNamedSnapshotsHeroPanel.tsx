...
type NamedSnapshotItem = { id: string; name: string; savedAt: string };

type ZsNamedSnapshotsHeroPanelProps = {
  exportLabel: string;
  setExportLabel: (value: string) => void;
  namedSaveName: string;
  setNamedSaveName: (value: string) => void;
  namedSnapshots: NamedSnapshotItem[];
...
export function ZsNamedSnapshotsHeroPanel({
  exportLabel,
  setExportLabel,
  namedSaveName,
  setNamedSaveName,
  ...
  onRestore,
  onDelete,
}: ZsNamedSnapshotsHeroPanelProps) {
  return (
    <div className="hero-named-grid hero-actions-tiered__named" aria-label="Pojmenované zálohy">
      ...
      <div className="hero-named-field hero-named-field--save">
        <button type="button" className="btn ghost btn--hero-named" onClick={onSave}>
          {NAMED_BACKUPS_SAVE_LABEL}
        </button>
      </div>
      ...
      <div className="hero-named-field hero-named-field--restore-delete">
        <button type="button" className="btn ghost btn--hero-named" onClick={onRestore}>
          {NAMED_BACKUPS_RESTORE_LABEL}
        </button>
        <button type="button" className="btn ghost btn--hero-named" onClick={onDelete}>
          {NAMED_BACKUPS_DELETE_LABEL}
        </button>
      </div>
    </div>
