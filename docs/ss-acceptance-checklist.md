...
Orientační podklad pro předání modulu `?view=ss` (verze **0.3.x**). Aplikace **nenahrazuje** oficiální výstup školy – slouží k orientaci a kontrole vstupů.
...
## Co aplikace v 0.3.x počítá / nepočítá
...
| Výsledková sekce                                         | `SsResultsSection` (`data-section="ss-vysledek"`)   | —                              |

## Příprava

1. Spusťte `npm run build`, `npm run test:golden` a `npm run test:e2e` (nebo ověřte poslední CI na `master`).
...
| 2026-06-02 | CI + contract | A–E regrese | OK (golden + E2E) | `SsResultsSection`, CI 0.3.6+ |
